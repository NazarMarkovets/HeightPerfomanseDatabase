...
from 
	`order` o
join 
	order_services card on card.fk_order_id = o.id
join 
	services se on card.fk_service_id = se.id
join 
	paper p on p.id = se.fk_service_paper
join 
	`status` st on st.id = o.fk_order_to_stat
join 
	paper_covering cov on cov.id = p.fk_paper_covering
join 
