...
from 
	`user` u, `role` r
where 
	r.id = fk_user_to_role AND fk_user_to_role = 2 
...
select 
    ...
    os.`name`,
    `user`.`name`, `user`.sname, `user`.phone
from 
    `order`, `user`, `status` os
where 
...
-- количество и сумарная цена услуг использующее один размер бумаги по стандарту ISO и тип покрытия матовый
/*
use inno;
select
	sum(s.Price)`summary ser price`, count(1) `count of services`,
    i.`name` ISO, i.height, i.width, cov.`name` `covering name`
from 
	paper p,
    paper_covering cov,
    services s,
